...
<head>
    ...
    <link rel="icon" href="{{ asset('/icon_logo.png') }}?v=2" type="image/png">
...
<body class="text-muji-ink antialiased min-h-screen flex flex-col relative bg-transparent overflow-x-hidden">
    ...
    @php
        ...
        // 2. Resolve URL
        $bgUrl = $rawBg;
        if (!\Illuminate\Support\Str::startsWith($bgUrl, 'http')) {
            $cleanPath = ltrim($bgUrl, '/');
            if ($cleanPath !== 'bg.jpg' && !\Illuminate\Support\Str::startsWith($cleanPath, 'storage/')) {
    ...
    <div id="bg-wrapper" class="fixed inset-0 z-[-1] overflow-hidden pointer-events-none select-none" style="background-color: var(--muji-base);">
        <div id="global-bg-element" class="absolute inset-y-0 bg-cover bg-center bg-no-repeat transition-all duration-700 ease-in-out" style="background-image: url('{{ $bgUrl ?: asset('bg.jpg') }}') ;
            ...
            <!-- Darkness Overlay -->
            <div class="absolute inset-0 transition-colors duration-700 ease-in-out" style="background-color: var(--bg-overlay);"></div>
        </div>
    </div>
        ...
        <div class="max-w-4xl mx-auto px-2 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <a href="{{ auth()->check() ? route('home', ['user' => auth()->user()]) : '/' }}" class="flex items-center gap-2 flex-shrink-0 group">
                <div class="w-10 h-10 rounded-lg bg-muji-base flex items-center justify-center shadow-muji-sm group-hover:scale-105 transition-transform overflow-hidden relative">
                    <img src="{{ asset('/icon_logo.png') }}?v=2" alt="Logo" class="w-full h-full object-contain p-1 z-10 relative">
                </div>
                        ...
                        @auth
                            ...
                            <div class="relative ml-1" id="userDropdownContainer">
                                ...
                                <div id="userDropdown" class="absolute top-14 right-0 w-52 bg-muji-paper rounded-[24px] shadow-2xl border border-muji-edge/50 py-2 hidden origin-top-right transition-all z-[100] overflow-hidden">
                                    ...
                                    <!-- Theme Toggle (Moon/Sun) - Now Tucked Away -->
                                    <button onclick="toggleTheme()" class="w-full flex items-center gap-3 px-4 py-2.5 text-left text-xs font-black text-muji-ash hover:text-muji-ink hover:bg-muji-base transition-all border-0 bg-transparent cursor-pointer">
                                        ...
                                        <span class="block dark:hidden">切換深夜模式</span>
                                        <span class="hidden dark:block">切換日間模式</span>
                                    </button>
    ...
    <div id="mobileMenuDrawer" class="fixed right-0 top-0 bottom-0 z-[2001] w-[85%] max-w-sm bg-muji-paper shadow-2xl flex flex-col sm:hidden rounded-l-[32px] overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div class="pt-10 mt-6 space-y-2 relative">
                ...
                @auth
                    <button onclick="toggleMobileMenu(false); safeOpenModal('globalProfileConfigModal')" class="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-muji-base text-muji-ash transition-all text-left border-0 bg-transparent cursor-pointer">
                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                    ...
                    <button onclick="toggleTheme()" class="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-muji-base text-muji-ash transition-all text-left border-0 bg-transparent cursor-pointer">
    ...
    <footer class="bg-muji-paper/70 border-t border-muji-edge h-16 mt-auto backdrop-blur-sm flex items-center justify-center">
        <div class="max-w-4xl mx-auto px-4 text-center text-muji-ash text-[10px] font-black tracking-[0.4em] uppercase">
    ...
    <script>
        ...
        @endif
            ...
            async function handleAjaxSubmit(event, form, modalId) {
                ...
                const noLoading = form.hasAttribute('data-no-loading') || submitBtn.hasAttribute('data-no-loading');
