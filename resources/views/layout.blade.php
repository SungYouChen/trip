...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col relative bg-transparent">
    @php
        // Determine which user's background and settings to use
        // Priority: Trip Owner (if set and has image) > Authenticated User (if has image) > Default
        if (isset($user) && !empty($user->background_image)) {
            $bgOwner = $user;
        } elseif (auth()->check() && !empty(auth()->user()->background_image)) {
            $bgOwner = auth()->user();
        } else {
            $bgOwner = $user ?? auth()->user() ?? null;
        }

        $finalBg = ($bgOwner && !empty($bgOwner->background_image)) ? $bgOwner->background_image : 'bg.jpg';
        
        // Ensure visual settings follow the bgOwner (consistent with the image shown)
        $bgOpacity = (($bgOwner->bg_opacity ?? 40)) / 100;
        $bgBlur = $bgOwner->bg_blur ?? 5;
        $bgStyle = $bgOwner->bg_style ?? 'full';
        $bgWidth = $bgOwner->bg_width ?? 45;
    @endphp
    ...
    <style id="bg-live-styles">
        ...
        @media (max-width: 768px) {
            :root { --bg-width: 100% !important; }
            #global-bg-element { left: 0 !important; transform: none !important; }
        }
    </style>
    ...
    <div id="bg-wrapper" class="fixed inset-0 z-[-10] overflow-hidden pointer-events-none select-none" style="background-color: #f5f4f2;">
        ...
        @endphp
        <div id="global-bg-element" 
             class="absolute inset-y-0 bg-cover bg-center bg-no-repeat transition-all duration-700 ease-in-out"
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            <div class="relative transform overflow-hidden bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl flex flex-col transition-all">
                ...
                <form action="{{ route('login.post') }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">電子郵件 Email</label>
                            <input type="email" name="email" required class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium" placeholder="admin@example.com">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">密碼 Password</label>
                            <input type="password" name="password" autocomplete="current-password" required class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all font-medium" placeholder="••••••••">
                        </div>
                        ...
                        <div class="text-sm text-center mt-6 text-gray-500 pt-6 border-t border-gray-100">
                            還沒有帳號？ <button type="button" onclick="safeCloseModal('loginModal'); setTimeout(() => safeOpenModal('registerModal'), 300)" class="text-indigo-600 font-bold hover:underline">立即註冊 Register Now</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="relative transform overflow-hidden bg-white rounded-3xl w-full max-w-lg p-8 shadow-2xl flex flex-col transition-all">
                ...
                <div class="max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                    <form action="{{ route('register.post') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">姓名 Name</label>
                                <input type="text" name="name" required class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium" placeholder="王小明">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">帳號 Username</label>
                                <input type="text" name="username" required class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium" placeholder="例如: elk" pattern="[a-zA-Z0-9_\-]+">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">電子郵件 Email</label>
                                <input type="email" name="email" required class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium" placeholder="example@gmail.com">
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <input type="password" name="password" required placeholder="設定密碼 Password" class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                                <input type="password" name="password_confirmation" required placeholder="確認密碼 Confirm" class="block w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-gray-900 shadow-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                            </div>
                            ...
                            <div class="text-sm text-center mt-6 mb-4 text-gray-500 pt-6 border-t border-gray-100">
                                已經有帳號了？ <button type="button" onclick="safeCloseModal('registerModal'); setTimeout(() => safeOpenModal('loginModal'), 300)" class="text-indigo-600 font-bold hover:underline">返回登入 Back to Login</button>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="px-8 py-10 overflow-y-auto custom-scrollbar scroll-smooth">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                            <input type="file" name="background_image" accept="image/*" class="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-bold file:bg-white file:text-slate-600 hover:file:bg-slate-50 cursor-pointer transition-all" id="bg-upload-input" onchange="this.form.submit()">
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 pt-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center">
                                <label class="text-sm font-bold text-slate-600">圖片透明度</label>
                                <span id="val-bg-opacity" class="text-xs font-mono text-slate-800 font-bold">{{ auth()->user()->bg_opacity }}%</span>
                            </div>
                            <input type="range" name="bg_opacity" id="range-bg-opacity" min="0" max="100" value="{{ auth()->user()->bg_opacity }}" 
                                   class="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-slate-600"
                                   oninput="previewBackground()">
                        </div>

                        <!-- 模糊度 -->
                        <div class="space-y-3">
                            <div class="flex justify-between items-center">
                                <label class="text-sm font-bold text-slate-600">圖片霧化 (Blur)</label>
                                <span id="val-bg-blur" class="text-xs font-mono text-slate-800 font-bold">{{ auth()->user()->bg_blur }}px</span>
                            </div>
                            <input type="range" name="bg_blur" id="range-bg-blur" min="0" max="20" value="{{ auth()->user()->bg_blur }}" 
                                   class="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-slate-600"
                                   oninput="previewBackground()">
                        </div>
                        ...
                        <div id="width-settings" class="space-y-3">
                            <div class="flex justify-between items-center">
                                <label class="text-sm font-bold text-slate-600">區塊顯示寬度</label>
                                <span id="val-bg-width" class="text-xs font-mono text-slate-800 font-bold">{{ auth()->user()->bg_width }}%</span>
                            </div>
                            <input type="range" name="bg_width" id="range-bg-width" min="5" max="100" value="{{ auth()->user()->bg_width }}" 
                                   class="w-full h-1.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-slate-600"
                                   oninput="previewBackground()">
                            <p class="text-[10px] text-gray-400 mt-1">※ 手機版本預設為滿版顯示，以達到最佳閱讀體驗。</p>
                        </div>

                        <div class="pt-4 space-y-3">
                            <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-3 rounded-2xl hover:bg-indigo-700 transition-all shadow-xl hover:shadow-indigo-200 transform active:scale-95">
                                儲存視覺設定
                            </button>
                            <button type="button" onclick="resetBgDefaults()" class="w-full bg-slate-50 text-slate-500 font-bold py-3 rounded-2xl hover:bg-slate-100 transition-all text-sm border border-slate-100">
                                ↺ 回復原始設定
                            </button>
                        </div>
                    </div>
    ...
    <header class="bg-white/70 shadow-sm sticky top-0 z-50 backdrop-blur-md">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <a href="{{ auth()->check() ? route('home', ['user' => auth()->user()]) : '/' }}" class="flex items-center gap-2 group">
                <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center shadow-md group-hover:scale-105 transition-transform overflow-hidden relative">
                ...
                <h1 class="text-lg sm:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 truncate max-w-[140px] xs:max-w-[200px] sm:max-w-none">
                    {{ isset($trip) ? $trip->name : 'Trip Planner' }}
                </h1>
            </a>

            <nav class="flex items-center justify-end gap-1 sm:gap-4 text-slate-400 font-medium">
                @if(isset($trip))
                    @if($isShared)
                        <a href="{{ route('trip.index_shared', ['token' => $trip->share_token]) }}" class="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2 px-2 py-1 rounded-lg hover:bg-slate-50 transition-all {{ request()->routeIs('trip.index_shared') ? 'text-slate-900 bg-slate-100' : '' }}">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
                    ...
                    @else
                        ...
                        <a href="{{ route('expenses.index', ['user' => $trip->user, 'trip' => $trip]) }}" class="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2 px-2 py-1 rounded-lg hover:bg-slate-50 transition-all {{ request()->routeIs('expenses.index') ? 'text-slate-900 bg-slate-100' : '' }}">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            <span class="text-[10px] sm:text-xs">消費</span>
                        </a>
                    @endif
                ...
                @if(!$isShared)
                    <a href="{{ auth()->check() ? route('home', ['user' => auth()->user()]) : '/' }}" class="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-2 px-2 py-1 rounded-lg hover:bg-slate-50 transition-all {{ request()->routeIs('home') ? 'text-slate-900 bg-slate-100' : '' }}">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
    ...
    <footer class="bg-white/70 border-t border-gray-100 py-8 mt-auto backdrop-blur-sm">
        <div class="max-w-4xl mx-auto px-4 text-center text-gray-400 text-sm">
            <p>2025 &copy; Honeymoon Trip to Japan</p>
        </div>
    </footer>
                ...
                @if(request()->routeIs('day.show') || request()->routeIs('day.show_shared'))
                    @auth
                        <button onclick="toggleSpeedDial(); if(typeof openEventModal === 'function') { openEventModal(); } else { safeOpenModal('eventDetailsModal'); }" class="flex items-center gap-2 group">
                            <span class="bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">新增行程</span>
                            <div class="w-12 h-12 bg-white text-slate-600 rounded-full shadow-lg border border-slate-100 flex items-center justify-center hover:bg-slate-50 transition-all hover:scale-105">
                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            </div>
                        </button>
                    @endauth
                @endif
                <button onclick="toggleSpeedDial(); safeOpenModal('expenseModal');" class="flex items-center gap-2 group">
                    <span class="bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">記錄消費</span>
                        <div class="w-12 h-12 bg-white text-slate-600 rounded-full shadow-lg border border-slate-100 flex items-center justify-center hover:bg-slate-50 transition-all hover:scale-105">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </div>
                </button>
            </div>
            <button onclick="toggleSpeedDial()" id="speedDialMain" class="w-14 h-14 bg-indigo-600 text-white rounded-full shadow-xl hover:bg-indigo-700 transition-all hover:scale-105 flex items-center justify-center relative transform rotate-0 duration-300">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="flex relative rounded-xl border border-gray-200 overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 bg-white group transition-all">
                                        ...
                                        <input type="number" step="0.01" id="expenseAmount" name="amount" required class="flex-1 w-full border-0 bg-transparent focus:ring-0 px-4 py-3 font-mono text-gray-900" placeholder="0.00">
                                    </div>
                                </div>
                                <div>
    ...
    <script>
        ...
        function toggleSpeedDial() {
            ...
            if (menu.classList.contains('hidden')) {
                ...
                mainBtn.classList.add('bg-indigo-700');
                mainBtn.classList.remove('bg-indigo-600');
            } else {
                menu.classList.add('hidden');
                icon.classList.remove('rotate-45');
                mainBtn.classList.remove('bg-indigo-700');
