                    ...
                    @if(!$isShared)
                        <div class="absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                            @auth
                                ...
                                <button onclick='openExpenseModal(@json($expense))' class="p-2 text-muji-ash hover:text-muji-oak hover:bg-muji-base rounded-full transition-all" data-tooltip="編輯">
                                ...
                                <form id="{{ $exTotalDelFormId }}" action="{{ route('expenses.destroy', ['user' => $trip->user, 'expense' => $expense->id]) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmDelete('刪除紀錄', '確定要刪除這筆開銷嗎？', '{{ $exTotalDelFormId }}')" class="p-2 text-muji-ash hover:text-red-500 hover:bg-red-50 rounded-full transition-all" data-tooltip="刪除">
